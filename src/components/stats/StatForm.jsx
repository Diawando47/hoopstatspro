...
export default function StatForm() {
  ...
  const defaultMatchId = modal?.data?.matchId ?? matches[0]?.id ?? ''
  ...
  async function handleSubmit() {
    ...
    // Upsert — replace if same match + player
    ...
    setLoading(false)
    closeModal()
    show('✅ Statistiques enregistrées !')
  }

  return (
    <>
      <div className="modal-title">📊 Saisir Statistiques</div>
      <div className="form-grid">
        ...
        <div className="form-group full">
          ...
          <select value={form.playerId} onChange={e => set('playerId', e.target.value)}>
            {players.map(p => (
              <option key={p.id} value={p.id}>{p.name} ({p.pos} — N°{p.number})</option>
            ))}
          </select>
        </div>
